      ...
      {pendingResetComicId !== null && pendingComic && (
        <div className="border-t border-neutral-100 bg-neutral-50/70 px-5 py-4">
          <div className="rounded-[24px] border border-neutral-200 bg-white p-4 shadow-sm">
            <h3 className="text-base font-black text-neutral-900">Reset Progress?</h3>
            <p className="mt-2 text-sm leading-relaxed text-neutral-600">
              Kamu akan menghapus seluruh progres pembelajaran pada komik{' '}
              <span className="font-black text-neutral-900">{pendingComic.title}</span>{' '}
              dan memulai kembali dari halaman pertama. Progress yang sudah dihapus tidak dapat dikembalikan.
            </p>
            <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:justify-end">
              <button
                type="button"
                onClick={() => setPendingResetComicId(null)}
                className="inline-flex items-center justify-center rounded-2xl border border-neutral-200 px-4 py-2.5 text-sm font-black text-neutral-700 transition hover:bg-neutral-50"
              >
                Batal
              </button>
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    showSnackbar('✅ Progress berhasil direset. Silakan mulai petualangan dari awal.', 'success');
                  } catch (error) {
                    const message = error instanceof Error ? error.message : 'Terjadi kesalahan saat reset progress.';
                    showSnackbar(`❌ Gagal mereset progress: ${message}`, 'error');
                  } finally {
