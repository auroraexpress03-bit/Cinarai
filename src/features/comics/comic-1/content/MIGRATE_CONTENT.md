TODO: Migrasi paket konten untuk Comic 1
...
- Setelah selesai, `src/features/comics/comic-1/index.ts` harus mengimpor lokal `./content` bukan `contentPackages` global.

Langkah singkat:
1. Buat file `packageContent.ts` berisi objek `LearningContentPackage` hanya untuk komik 1.
2. Uji integrasi dengan `LearningEngine` memastikan `comicModule.packageContent` memuat data yang sama.
3. Hapus ketergantungan runtime ke `getLearningContentPackage(1)`.
