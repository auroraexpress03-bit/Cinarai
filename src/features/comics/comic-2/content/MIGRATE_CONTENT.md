TODO: Migrasi paket konten untuk Comic 2
...
- Setelah selesai, `src/features/comics/comic-2/index.ts` harus mengimpor lokal `./content` bukan `contentPackages` global.

Langkah singkat:
1. Buat file `packageContent.ts` berisi objek `LearningContentPackage` hanya untuk komik 2.
2. Uji integrasi dengan `LearningEngine` memastikan `comicModule.packageContent` memuat data yang sama.
3. Hapus ketergantungan runtime ke `getLearningContentPackage(2)`.
