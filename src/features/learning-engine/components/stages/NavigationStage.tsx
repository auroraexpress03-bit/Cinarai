      ...
      {isComic2 ? (
        <>
          <div className="space-y-2">
            <p className="text-sm font-semibold uppercase tracking-[0.24em] text-primary-600">CINARAI Navigation</p>
            <h1 className="text-2xl font-black text-neutral-900 sm:text-3xl">Jelajahi objek Candi Penataran</h1>
            <p className="max-w-2xl text-sm leading-relaxed text-neutral-600">Pilih objek yang muncul pada komik. Setiap tombol akan menampilkan bangun datar, penjelasan sederhana, dan AI Tutor yang fokus pada Candi Penataran.</p>
          </div>

          <section className="overflow-hidden rounded-[20px] border border-neutral-200 bg-white p-4 shadow-sm sm:p-5">
            <div className="space-y-2">
              <h2 className="text-lg font-black text-neutral-900">Objek Utama</h2>
              <p className="text-sm text-neutral-600">Lihat gambaran awal objek yang paling dekat dengan cerita komik sebelum masuk ke penjelasan detail.</p>
            </div>
            {candiEmbed ? (
              <div className="mt-4 overflow-hidden rounded-[16px] border border-neutral-200">
                <div className="relative aspect-[16/9] w-full">
                  <iframe
                    src={candiEmbed}
                    title={candiEntry?.title ?? 'Objek Utama'}
              ...
              <div className="mt-4 overflow-hidden rounded-[16px] border border-neutral-200 bg-neutral-50 p-3">
                <Image
                  src={candiQrImage}
                  alt={candiEntry?.title ?? 'Objek Utama'}
                  width={800}
                  height={800}
                  quality={100}
                  priority
          ...
          <section className="space-y-3">
            <div className="flex items-center justify-between gap-2">
              ...
              <span className="rounded-full bg-primary-50 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-primary-700">Bangun Datar</span>
            </div>
              ...
              {objects.map((obj) => {
                ...
                return (
                  <div key={obj.id} className={[
                    'flex h-full flex-col justify-between rounded-[16px] border bg-white p-3 shadow-sm transition-all',
                    isOpened ? 'border-primary-300 bg-primary-50/70 shadow-primary-100' : 'border-neutral-200',
                  ].join(' ')}>
                    <div className="space-y-2">
                      {obj.navImage ? (
                        <div className="relative mb-2 h-28 overflow-hidden rounded-xl border border-neutral-200 bg-neutral-50">
                          <Image
                            src={obj.navImage}
                            alt={obj.title}
                            width={600}
                      ...
                      <div className="flex items-start justify-between gap-2">
                        ...
                        {obj.shapeName ? (
                          <span className="rounded-full bg-white px-2 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-neutral-600">{obj.shapeName}</span>
                        ) : null}
                      </div>
